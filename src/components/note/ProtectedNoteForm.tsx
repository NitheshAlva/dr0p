...
export default function ProtectedNoteForm({ name }: ProtectedNoteFormProps) {
  ...
  const onSubmit = async (data: z.infer<typeof protectedSchema>) => {
    ...
    try { 
      const resp = await getNoteAction(data.name, data.password)
      ...
      setIsVerified(true)
      setNote({name: resp.note.name, content: resp.note.content, expiresAt: resp.note.expires_at, isProtected: true})
